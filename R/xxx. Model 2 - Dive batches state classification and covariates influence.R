...
library(pacman)
p_load(sp, sf, magrittr, lubridate, tidyverse, geosphere, momentuHMM, ggplot2)
...
model.selection <- data.frame("formula" = "null model", "log-likelihood" = -ms$mod$minimum, 
                              "AIC" = AIC(ms), "BIC" = -2*(-ms$mod$minimum)+length(ms$mod$wpar)*log(length(data$ID)))
model.output <- model.selection[NA,]
...
for(x in 1:length(formulas.list)){
  ...
  model.output$formula[x] <- as.character(formulas.list[x])
  model.output$log.likelihood[x] <- -m$mod$minimum
  model.output$AIC[x] <- AIC(m)
  model.output$BIC[x] <- -2*(-m$mod$minimum)+length(m$mod$wpar)*log(length(data$ID))
  model.selection[x] <- rbind(model.selection, model.output)
  print(x)
}
...
plotSpatialCov(m2, spatialCov = Longbrick$X90)
